...
%% API
-export([initDB/0, registerUser/2, joinChat/2, addMessage/3, getUsers/0, getChats/0, getChatUsers/1, getChats1/1,getUsers/1, getUserMessages/2, getUserMessages1/1, getChatMessages/1, storeDB/2, getDB/1, getDBTwo/1, deleteDB/1, stopDB/0, infoDB/0]).
...
%%%------------------------------Chat Initialization------------------------------%%%
% User registration from tcp.erl uses this function.
...
getUserMessages1(User) ->
  AF = fun() ->
    Query = qlc:q([X || X <- mnesia:table(message), (X#message.userName =:= User)]),
    Results = qlc:e(Query),
    lists:map(fun(Item) -> Item#message.msg end, Results)
       end,
  {atomic, Comments} = mnesia:transaction(AF),
  Comments.
%%%-------------------------------------------------------------------------------%%%
